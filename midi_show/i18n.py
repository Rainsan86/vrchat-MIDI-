"""Internationalization (i18n) support for VRChat MIDI Player."""
...
_TRANSLATIONS: Dict[str, Tuple[str, str]] = {
    # ── Window title ──
    "window.title": (
        "VRChat MIDI Player 1.2.0",
        "VRChat MIDI 播放器 1.2.0",
    ),
    # ── Top buttons & labels ──
    ...
    # ── Output: Virtual MIDI ──
    "output.virtual_midi": (
        "Virtual MIDI Port (VRChat World)",
        "虚拟 MIDI 端口（VRChat 世界）",
    ),
    "output.port": ("Port:", "端口:"),
    ...
    "library.empty": (
        "Library is empty.\nClick 'Add Files' or drag & drop MIDI files here.",
        "曲库为空。\n点击「添加文件」或拖拽 MIDI 文件到此处。",
    ),
    "library.add_files": ("+ Add Files", "＋ 添加文件"),
    "library.add_folder": ("+ Add Folder", "＋ 添加文件夹"),
    "library.remove": ("Remove Selected", "移除选中"),
    "library.remove_selected": ("Remove Selected", "移除选中"),
    ...
    "btn.always_on_top_off": (
        "📌 Unpin",
        "📌 取消置顶",
    ),
    "status.always_on_top_on": (
        "Window pinned on top",
        "窗口已置顶",
    ),
    "status.always_on_top_off": (
        "Window unpinned",
        "窗口已取消置顶",
    ),
    ...
    "btn.reset": ("Reset", "重置"),
    "status.reset": ("Speed and transpose reset to defaults", "速度和移调已重置为默认值"),
    # ── Note count display ──
    "note_count.format": (
    ...
    "tab.appearance": ("Background", "界面背景"),
    "appearance.section": ("Custom Background", "自定义背景"),
    "appearance.hint": (
        "Choose an image as the window wallpaper. Content cards stay frosted so text stays readable.",
        "选择一张图片作为窗口壁纸。内容卡片会使用半透明磨砂底，既能看到背景，也能保证文字可读。",
    ),
    "appearance.choose": ("Choose Image", "选择图片"),
    "appearance.clear": ("Clear Background", "清除背景"),
    "appearance.none": ("No custom background", "未设置自定义背景"),
    "appearance.dialog_title": ("Select background image", "选择背景图片"),
    "appearance.image_files": ("Images", "图片文件"),
    "status.bg_set": ("Background updated: {name}", "已设置背景: {name}"),
    "status.bg_cleared": ("Custom background cleared", "已清除自定义背景"),
    "status.bg_failed": ("Failed to load image: {path}", "背景图片加载失败: {path}"),
    # ── Help tab content (zh) ──
    "help.content": (
        "VRChat MIDI Player\n\n"
        ...
        "   - Install LoopMIDI (free) or LoopBe1 (for Win11) and create a virtual port\n"
        ...
        "4. Try example VRChat world:\n"
        "   Search 'MIDI Piano' in VRChat world menu",
        "VRChat MIDI 播放器\n\n"
        "如何在 VRChat 中使用：\n\n"
        "1. 本地音频：\n"
        "   通过电脑扬声器播放 MIDI。\n"
        ...
        "   - 安装 LoopMIDI（免费）或者 LoopBe1（Win11安装这个）并创建一个虚拟端口\n"
...
def language_text_map(from_lang: str, to_lang: str) -> Dict[str, str]:
    """Map UI strings from one language to another (for in-place refresh)."""
    mapping: Dict[str, str] = {}
    if from_lang == to_lang:
        return mapping
    for en, zh in _TRANSLATIONS.values():
        old = zh if from_lang == "zh" else en
        new = zh if to_lang == "zh" else en
        if old and old != new:
            mapping[old] = new
    return mapping
